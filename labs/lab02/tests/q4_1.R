  ...
  cases = list(
    ottr::TestCase$new(
      ...
      code = {
        ...
        bills_vec <- c(20.12, 39.90, 31.01, 25.53, 48.99)
        expected <- bills_vec * 1.2
        if (!exists("total_charges") || is.null(total_charges)) {
          testthat::fail("You haven't changed `total_charges` from NULL yet.")
        } else if (!is.null(msg <- check_numeric(total_charges, "`total_charges`", n = 5))) {
          if (is.numeric(total_charges) && length(total_charges) == 1) {
            testthat::fail(paste0(msg, " You want one total per bill (that's Question 4.1); the grand total comes in 4.2."))
          } else {
            testthat::fail(msg)
          }
        } else if (isTRUE(all.equal(unname(total_charges), bills_vec * 0.2))) {
