  ...
  cases = list(
    ottr::TestCase$new(
      ...
      code = {
        ...
        expected <- sum(bills_vec * 1.2)
        if (!exists("sum_of_charges") || is.null(sum_of_charges)) {
          testthat::fail("You haven't changed `sum_of_charges` from NULL yet.")
        } else if (!is.null(msg <- check_numeric(sum_of_charges, "`sum_of_charges`"))) {
          if (is.numeric(sum_of_charges) && length(sum_of_charges) == 5) {
            testthat::fail(paste0(msg, " That's still the vector of charges. Use sum() to collapse it to one number."))
          } else {
            testthat::fail(msg)
          }
        } else if (isTRUE(all.equal(sum_of_charges, sum(bills_vec)))) {
